...
4. **"Confirm the film/show" and "confirm the timestamp" are separate, cheap-to-redo steps** in the Discord flow — users can back up one step without restarting `/cinesnip` from scratch. **Not yet actually implemented as of the V2 quote-matching work**: Cancel on either `ConfirmView` (film step) or `QuoteMatchView` (quote-match step) in `app/bot/cogs/gif.py` ends the whole interaction rather than backing up — a full `/cinesnip` restart is currently required either way. Before implementing "back up," decide what it concretely means here: Discord slash-command args (`film`/`quote`/`timecode`) can't be edited post-invocation, so "back up to re-pick the film" and "back up to try a different quote" need different UI (e.g. a modal for a new quote, vs. re-showing film search results) — worth a deliberate design pass, not a quick patch.
...
- **Known redundancy, not yet fixed**: a single `/cinesnip film:X quote:Y` invocation re-fetches the same Plex movie metadata via `PlexClient.get_movie()` three separate times (once each in `/resolve`, `/resolve-quote`, `/render` — each a real network round-trip to Plex) instead of once. A clean fix needs a real decision, not a quick patch: either add a short-lived in-process cache in `PlexClient` keyed by `rating_key`, or have the bot thread the already-fetched movie data through instead of re-requesting by key each step — the latter would mean exposing more than just `rating_key` across the worker's HTTP API, which is worth weighing against the Section 9 principle of keeping media access confined to the worker.
