...
- **`docs/design/fts5-search-migration.md`** — full design + real-scale measurements for the SQLite+FTS5 search migration (✅ built and running against the real library).
...
- **Subtitle cache** — persisted per Plex media GUID, so a film's subtitles are only ever extracted once. This cache doubles as the corpus for library-wide quote search (Section 5) — it's built up lazily/opportunistically as titles get touched by any flow, never by proactively indexing the whole library up front. **✅ Consolidated into SQLite+FTS5** (`cache/quote_index.db`, `app/worker/search_index.py`) — subtitle text itself now lives in this DB's `entries`/`entries_fts` tables, not flat per-title JSON cache files; the older `quote_index.py` bookkeeping tables in the same file are a separate, narrower pointer/status store (sync progress, no-subtitle titles), not where subtitle text lives. `/snip-search` enumerates cached titles via `search_index.list_titles()` rather than scanning/parsing every cache file. Full design, real-scale measurements, and a real correctness bug this migration fixed (an entry-vs-title `LIMIT` mistake in an early implementation): `docs/design/fts5-search-migration.md`. Shared with TV episodes too (both flow through the same generic `/render`/`/resolve-quote` — Section 4), so `/snip-search`'s own handler filters back down to movie-only libraries at read time rather than the index itself being movie-only.
...
- **✅ Fixed: `/snip-search`'s "instant regardless of library size" promise was measured to break down at real full-library scale** (~40s against an 11,463-title cache, fuzzy-scoring-bound) — this is exactly what motivated the SQLite+FTS5 migration below, and it's resolved now that migration has shipped (see the section immediately below for real post-migration numbers, including a real correctness/perf bug the migration itself introduced and then fixed at real scale).

### ✅ Built: the subtitle cache is consolidated into SQLite + FTS5

Full design, real measurements from the original 1,000-title prototype (search time 40s → 2ms–740ms, size 3.6GB → 1.57GB), the FTS5 typo-recall tradeoff and its required fallback, the alternatives ruled out (trigram tokenizer, thread parallelism, external search engines), **and the real full-library-scale findings from actually shipping it** (an entry-vs-title `LIMIT` bug that briefly reintroduced an O(corpus) cost despite the index, plus a per-caller scoping fix `/snip-tv`'s whole-show search needed): **`docs/design/fts5-search-migration.md`**. `scripts/build_full_cache.py` and `library_sync.py` write into this index directly; `scripts/migrate_to_fts5.py` backfills an existing install's legacy JSON cache into it (see README's "Upgrading an existing install").
...
- **Library-wide quote search (`/snip-search`, V2)**: searches the subtitle cache described in Section 1, not the live filesystem, so its scope is exactly "titles CineSnip has already parsed via any flow" — which starts small and grows with normal use. Two tiers, so a `/snip-search` call never surprises the user with a multi-minute wait:
  1. **✅ Built — default**: search only already-cached titles. Fast: an FTS5 pre-filter (`search_index.search_entry_ids()`) narrows a query to a bounded set of candidate subtitle-entry rows before any fuzzy-scoring happens, so search time no longer grows with library size the way a full scan would. Implementation: `GET /search-quote` (`app/worker/api.py`) enumerates cached titles via `search_index.list_titles()`, then `search_cached_library()` (`app/worker/library_search.py`) expands each FTS5 hit into a small adjacent-cue window and reuses `find_quote_matches()` unchanged against just those windows (see `docs/design/fts5-search-migration.md` for why this has to be entry-level windowing, not "every entry of a survivor title"). **Diversity-first ranking, not a hard one-per-title cap**: every title's best-scoring line competes for a results slot before any title's second-best line does, but unfilled slots backfill with each title's next-best line (up to `quote_match.library_per_title_limit`, default 3) — never at the expense of a better match from another title.
...
- **✅ Search moved from a per-title disk-cached-candidates design to SQLite+FTS5.** Normalizing subtitle lines and building match candidates used to be 53% of total search time, so an earlier version cached them to disk per title (`cache/<guid-digest>.candidates.json`) — a real win at small scale, but that JSON-based design itself stopped scaling once the whole library was actually built out (deserializing candidates got as expensive as the fuzzy scoring it was avoiding). Superseded by an FTS5 pre-filter that narrows a query to a few thousand candidate subtitle lines before any candidate-building/scoring happens at all, so there's nothing per-title left to precompute or cache. Full design, the numbers behind the switch, and a real full-library correctness bug found and fixed while shipping it: `docs/design/fts5-search-migration.md`.
