...
- **✅ Fixed a real path-mapping bug found via the full-library cache build (Section 5): Windows' extended-length path prefix wasn't stripped before matching.** 14 real titles (a Borat film, several multi-episode *Avatar: The Last Airbender* files) failed with "No path mapping configured" despite an otherwise-correct `path_mappings` entry. Root cause: Plex reports the `\\?\` prefix (a Win32-API-only escape sequence letting a path exceed the 260-char MAX_PATH limit) verbatim for titles whose combined path/filename is long enough — `resolve_container_path()`'s `_normalize()` (`app/worker/path_mapper.py`) only converted backslashes to forward slashes, so `\\?\D:\...` became `//?/D:/...` and no longer started with the configured `D:/...` prefix. Fixed by stripping `//?/` (and the `//?/UNC/` variant, for an extended-length real network path) after backslash normalization, before prefix-matching. Verified against both real failing titles: Borat now resolves and renders correctly; a real multi-episode Avatar file (S03E10) now resolves via `/resolve-episode` and its subtitles extract correctly.
...
**✅ `scripts/build_full_cache.py` made incremental — a real prototype of what "auto-cache new/changed titles" should look like regardless of backend.** Re-running the full build after adding new content used to mean a ~30-minute sweep touching every title again. Fixed by skipping any title whose subtitle cache file already exists (`cache_path_for_guid(...).exists()` — no read/parse, no Plex path resolution beyond what enumeration already returns for free) unless `--force` is passed. Verified against the real library immediately after the path-mapping fix above: re-running against all 11,463 titles took **1 second**, correctly skipping the 11,451 already cached and only doing real work for the 12 titles still missing (the 14 originally broken by the UNC bug, minus 2 already picked up by manual verification while fixing it) — 0 errors, 0 skips.

This is deliberately the shape any future "automatic" sync should take, whatever triggers it: **enumerate → cheap existence/freshness check → do real work only for what's missing.** Two ways to *trigger* it were considered and set aside for now — a Plex webhook (real-time, but Plex Pass–gated and would need the worker to accept an inbound connection, cutting against Section 9's loopback-only design) and a scheduled/cron job (works, but is proactive background work the project has otherwise deliberately avoided). A cheap manually-triggered incremental sync covers "a few new films a day" well enough that automating the trigger isn't buying much yet — revisit if that changes. Whenever the SQLite+FTS5 migration above lands, this script's *structure* carries over unchanged; only the existence check (`cache_path_for_guid(...).exists()` → `SELECT 1 FROM titles WHERE guid=?`) and the write (a JSON dump → an `INSERT`) need updating.

- **✅ Built: opt-in automatic library sync (`config.yaml`'s `library_sync.enabled`), addressing this same section's "revisit if that changes" note above** — the trigger is now automatable after all, once real testing showed the previously-assumed "proactive background work" cost was wrong. New module `app/worker/library_sync.py`; `scripts/build_full_cache.py` above became a thin CLI wrapper around its shared `sync_one_title()` so there's one implementation, not two.
  - **Change detection via Plex's own `section.updatedAt`** (not a webhook, still ruled out for the reasons above) — verified with four real tests against the live server (Refresh Metadata + Analyze, on both a movie and a TV episode): none moved `updatedAt`, confirming it only fires on genuine scanner-detected changes, not routine per-title actions. **Real bug found and fixed while building this**: getting a *fresh* value needs `section.reload()` on the already-held section object — confirmed directly that re-fetching via `PlexServer.library.sections()`/`.section(name)` returns the exact same cached Python object (`is` identity check confirmed no network call happens), so a naive "just re-fetch the section" implementation would have silently read stale data forever. **Second real bug, same investigation**: `section.updatedAt` is a `datetime` object, not the raw int the rest of the app (SQLite storage, equality comparisons) needs — Python 3.12 no longer auto-adapts `datetime` for sqlite3, so storing it directly would have failed or silently misbehaved. Fixed by converting to `int(updatedAt.timestamp())` in `PlexClient.current_section_updated_ats()`, the single place this conversion needs to happen.
  - **Removal detection + two-layer safety guard**, per the user's explicit requirement that automatic deletion must not be triggerable by something that only *looks* like removal. New `quote_index.remove_cached_title()`/`list_cached_titles_for_library()`, `subtitles.delete_cached_subtitles()`, `quotes.delete_cached_candidates()`. Layer 1 (mount check): every configured `path_mappings` root for a library must be reachable and non-empty before any deletion is considered. Layer 2 (spot check): a sample of titles Plex still lists as present must also actually resolve to a real file on disk. Either failing skips cleanup for that whole library *and leaves its stored `updatedAt` unchanged*, so the next cycle still sees "changed" and retries — additions still proceed either way, since those already fail safe per-title. A Plex enumeration call that raises is handled distinctly from "returned zero/few items" — an exception never triggers removal logic.
  - Verified against the real library, not just unit tests: a real first sync cycle against the full 10,692-title index (nothing actually removed from Plex since the cache was built) correctly reported `removed=0` across all 3 libraries with the index count unchanged before/after (10,692 → 10,692); a second immediate run correctly short-circuited via the cheap `updatedAt` check in **0.01s**; and the scheduled task itself was confirmed running for real inside the app process (temporarily enabled, rebuilt, confirmed it fires its immediate first cycle on startup and logs cleanly without blocking the bot/worker) before being reverted back to disabled.
  - Config: `LibrarySyncDefaults` (`app/settings.py`) — `enabled` (default `false`) and `interval_hours` (default `24`) are the only two knobs; safety-guard constants (spot-check sample size) are deliberately not configurable. Runs as an extra coroutine in `app/main.py`'s existing `asyncio.gather()`, reusing the worker's already-constructed `PlexClient` (`worker_app.state.plex`) rather than opening a second Plex connection — no new container/cron needed.
...
7. **Library auto-sync toggle** (once built — see Section 5's build notes):
