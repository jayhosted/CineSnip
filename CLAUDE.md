...
- **✅ Diagnosed: the earlier "possible sync drift" was per-title subtitle desync, not a CineSnip seek bug.** Real testing after burn-in shipped found "Snatch" consistently ~1s off, while "Obsession" was fine — same code path for both, so a real seek bug would have hit both equally. Confirmed directly: `Snatch (2000)...en.srt`'s file mtime was days newer than its `.mkv` (a recent Bazarr re-fetch, likely from a differently-synced release), while `Obsession`'s `.srt`/`.mkv` mtimes matched. Verified with `ffmpeg -af silencedetect` against the raw audio at Snatch's first line (isolated by ~9.7s of clean silence, so unambiguous): real speech onset ~36.77s vs. the `.srt`'s claimed 37.70s — a ~0.94s offset, matching what was heard by ear. A **constant** offset (not growing deeper into the file, which is what a frame-rate mismatch would look like) is the signature of a mis-synced subtitle release, not a timestamp-math bug — nothing to fix in `ffmpeg.py`'s seek code. No general mitigation planned for V2 (would mean per-title automatic subtitle-sync detection/correction, a real feature of its own); if it recurs, re-fetching/replacing that title's subtitle via Bazarr is the actual fix.
