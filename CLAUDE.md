...
- **Stretch goal, not yet built — 3D format handling.** Confirmed against this developer's own `3D` library: real-world files use more than one packing (over/under *and* side-by-side both present), so a single hardcoded crop doesn't work across the whole library — right now, generating a clip from anything in that library produces an unusable squished/doubled frame, full stop. Likely design: tag each `LibraryConfig` entry (`app/settings.py`) with an optional 3D packing mode (e.g. `three_d_format: side_by_side | over_under | none`), since the packing is a property of how a *library* was encoded/organized, not something to detect per-file. `ClipRenderer` (`app/worker/ffmpeg.py`) would then need a crop-to-one-eye step (`crop=iw/2:ih:0:0` for SBS-left, `crop=iw:ih/2:0:0` for OU-top, etc.) inserted into the filter graph before scaling, driven by that config value, so a 3D library's output is a normal flat, usable clip instead of the raw squished frame. Worth a real design pass (which eye to default to, whether to expose it as a per-request override) rather than a bolt-on — flagged here so it doesn't get lost, not scheduled against a specific version yet.
...
- **Phase 1 — 3D crop/format handling** (Section 3 stretch goal). Small and self-contained (a `three_d_format` tag on `LibraryConfig` plus a crop step in `ClipRenderer`'s filter graph), and already confirmed broken against this developer's real `3D` library today — doesn't block or depend on anything else below, so it's cleared first rather than left flagged indefinitely.
- **Phase 2 — TV show support** (episode-specific and whole-show search, Section 4). The biggest remaining functional gap; reuses the film flow's subtitle/matching infra (Section 5) with an extra Show → Season → Episode layer. Independent of Whisper and the web app.
...
- **Phase 6 — `/cinesnip-search` Tier 2** (Section 5: extend search to not-yet-cached titles). Deliberately deferred until a real progress/ETA UX exists rather than a silent multi-minute hang — Phase 5 is what builds that, so Tier 2 reuses it instead of inventing a throwaway Discord-only progress hack.
